...
BEGIN
  ...
  IF v_match_status IN ('COMPLETED', 'completed', 'FINISHED', 'finished', 'CANCELLED', 'cancelled') THEN
    RAISE EXCEPTION 'Match is already %, cannot publish results again.', v_match_status;
  END IF;
  ...
  -- 3.5 Detect column existence for profiles
  ...
  LOOP
    ...
    IF v_prize > 0 AND NOT EXISTS (
      ...
      -- Dynamic update for profiles balance
      ...
      -- Create audit record (WINNING_PRIZE per request)
  ...
  -- 6. Finalize Tournament and set results_published
  ...
  RETURN jsonb_build_object(
    'success', true, 
    'message', 'Results published successfully. Status updated to COMPLETED and prizes credited.',
