...
export const CouponManagement: React.FC<CouponManagementProps> = ({
  ...
  return (
    <div className="space-y-5 animate-in fade-in pb-16 md:pb-6">
      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-2xl bg-gradient-to-r from-purple-900/90 via-indigo-950/90 to-purple-950 border border-[#29252A] shadow-2xl">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#B0ACB0] font-semibold">Secure Database RPC System</span>
          </div>
          <h2 className="text-xl font-black text-white mt-1.5 flex items-center gap-2">
            <Ticket className="w-6 h-6 text-[#C9A34E]" /> Coupon & Promo Code Manager
          </h2>
          <p className="text-xs text-[#B0ACB0]/80 mt-0.5">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button
            ...
            className="p-2.5 rounded-xl bg-[#1B181C] hover:bg-[#FF3048] text-[#B0ACB0] border border-[#29252A] hover:border-amber-400/50 transition disabled:opacity-50"
            title="Refresh Coupon List"
          >
            <RefreshCw className={`w-4 h-4 ${isLoading ? 'animate-spin text-[#C9A34E]' : ''}`} />
          </button>
      ...
      {successMessage && (
        <div className="p-3.5 rounded-xl bg-[#350A12]/80 border border-[#29252A] text-emerald-200 text-xs flex items-center justify-between gap-2 shadow-lg">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-[#C9A34E] flex-shrink-0" />
            <span>{successMessage}</span>
          </div>
          <button onClick={() => setSuccessMessage(null)} className="text-[#C9A34E] hover:text-white text-xs">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-[#0D0B0D] text-white text-xs rounded-xl border border-[#29252A] focus:border-[#C9A34E] focus:outline-none transition placeholder-purple-400/60"
          />
        </div>

        {/* Filter Tabs */}
        <div className="flex items-center p-1 bg-[#0D0B0D] rounded-xl border border-[#29252A]">
          {(['all', 'active', 'inactive'] as const).map((st) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold rounded-lg uppercase transition ${
                filterStatus === st
                  ? 'bg-[#141215] text-[#C9A34E] border border-[#29252A]/60 shadow-sm'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading && coupons.length === 0 ? (
          <div className="col-span-full p-12 rounded-2xl bg-[#0D0B0D] border border-[#29252A] text-center text-[#B0ACB0] text-xs">
            <RefreshCw className="w-6 h-6 text-[#C9A34E] animate-spin mx-auto mb-2" />
            Loading coupon database...
          </div>
        ) : filteredCoupons.length === 0 ? (
          <div className="col-span-full p-12 rounded-2xl bg-[#0D0B0D] border border-[#29252A] text-center text-[#B0ACB0] text-xs space-y-2">
            <Ticket className="w-10 h-10 text-[#E21B36]/60 mx-auto" />
            <p className="font-bold text-white text-sm">No coupons found</p>
            <p className="text-[#B0ACB0]/80 text-xs">
          ...
          filteredCoupons.map((c) => (
            <div
              key={c.id}
              className={`p-4.5 rounded-2xl bg-[#0D0B0D] border transition flex flex-col justify-between space-y-4 shadow-xl ${
                c.isActive ? 'border-[#29252A] hover:border-amber-400/60' : 'border-[#29252A]/80 opacity-70'
              }`}
            >
              <div className="space-y-3">
                {/* Header Row */}
                <div className="flex items-center justify-between pb-2.5 border-b border-[#29252A] gap-2">
                  <span className="font-mono text-base font-black tracking-wider text-[#C9A34E] bg-[#0D0B0D]/80 px-3 py-1 rounded-xl border border-[#29252A] shadow-inner">
                    {c.code}
                  </span>
                  <button
                    ...
                    className={`px-2.5 py-1 text-[10px] font-extrabold rounded-lg uppercase border transition flex items-center gap-1 ${
                      c.isActive
                        ? 'bg-[#350A12]/80 text-[#C9A34E] border-[#29252A] hover:bg-emerald-900'
                        : 'bg-rose-950/80 text-rose-400 border-rose-800 hover:bg-[#4A0D16]'
                    }`}
                  >
                ...
                {/* Description */}
                {c.description && (
                  <p className="text-xs text-[#B0ACB0]/90 italic bg-[#0D0B0D]/40 p-2 rounded-lg border border-[#29252A]/30">
                    "{c.description}"
                  </p>
                )}

                {/* Requirements & Rewards */}
                <div className="space-y-2 text-xs bg-[#141215]/60 p-3 rounded-xl border border-[#29252A]">
                  <div className="flex items-center justify-between">
                    <span className="text-[#B0ACB0] font-semibold">Reward Amount:</span>
                    <strong className="text-[#C9A34E] font-black text-sm">₹{c.rewardAmount || c.discountValue || 0} Bonus</strong>
                  </div>

                  <div className="flex items-center justify-between pt-1 border-t border-[#29252A]">
                    <span className="text-[#B0ACB0] font-semibold flex items-center gap-1">
                      <Info className="w-3 h-3 text-[#777278]" />
                      Eligible Deposit History:
                    </span>
                    <strong className="text-[#C9A34E] font-extrabold">₹{c.minDepositAmount ?? c.minDeposit ?? 0}+</strong>
                  </div>

                  <div className="flex items-center justify-between pt-1 border-t border-[#29252A] text-[11px]">
                    <span className="text-[#777278]">Usage Count:</span>
                    <strong className="text-[#B0ACB0]">
                ...
                {/* Dates */}
                <div className="space-y-1 text-[10px] text-[#B0ACB0]/80">
                  <div className="flex items-center justify-between">
                    <span>Starts:</span>
                    <span className="font-mono text-[#B0ACB0]">{formatDate(c.startsAt)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Expires:</span>
                    <span className="font-mono text-[#B0ACB0]">{formatDate(c.expiresAt || c.expiryDate)}</span>
                  </div>
                </div>
              </div>

              {/* Actions */}
              <div className="flex items-center gap-2 pt-2.5 border-t border-[#29252A]">
                <button
                  onClick={() => handleOpenEdit(c)}
                  disabled={isSubmitting}
                  className="flex-1 py-1.5 rounded-xl bg-[#1B181C] hover:bg-[#FF3048] text-[#B0ACB0] text-xs font-bold flex items-center justify-center gap-1.5 transition border border-[#29252A]/40"
                >
                ...
                <button
                  ...
                  disabled={isSubmitting}
                  className="p-2 rounded-xl bg-[#350A12] hover:bg-[#4A0D16] text-rose-400 border border-[#350A12] transition"
                  title="Delete Coupon"
                >
      ...
      {deletingCoupon && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="max-w-sm w-full bg-[#0D0B0D] p-5 rounded-2xl border border-[#350A12] shadow-2xl space-y-4 animate-in zoom-in-95">
            ...
            <p className="text-xs text-[#B0ACB0] leading-relaxed">
              Are you sure you want to delete coupon <strong className="text-[#C9A34E] font-mono">{deletingCoupon.code}</strong>? This operation cannot be undone.
            </p>
            ...
            <div className="flex items-center gap-2 pt-2">
              <button
                ...
                disabled={isSubmitting}
                className="flex-1 py-2.5 bg-[#0D0B0D] hover:bg-[#141215] text-[#B0ACB0] text-xs font-bold rounded-xl transition"
              >
                Cancel
              </button>
              <button
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-[#0D0B0D] p-5 rounded-2xl border border-[#29252A] shadow-2xl space-y-4 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-[#29252A]">
              <h3 className="font-extrabold text-base text-white flex items-center gap-2">
                <Ticket className="w-5 h-5 text-[#C9A34E]" />
                {editingCoupon ? 'Edit Coupon' : 'Create New Coupon'}
              </h3>
              <button
                onClick={() => setShowModal(false)}
                disabled={isSubmitting}
                className="text-[#777278] hover:text-white text-xs font-bold"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              {/* Code */}
              <div>
                <label className="block text-[11px] font-bold text-[#B0ACB0] uppercase mb-1">
                  Coupon Code <span className="text-[#C9A34E]">*</span>
                </label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-[#141215] text-white text-xs px-3.5 py-2.5 rounded-xl border border-[#29252A] focus:border-[#C9A34E] focus:outline-none uppercase font-mono font-bold tracking-wider"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#141215] text-white text-xs px-3.5 py-2.5 rounded-xl border border-[#29252A] focus:border-[#C9A34E] focus:outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-[#B0ACB0] uppercase mb-1">
                    Reward Amount (₹) <span className="text-[#C9A34E]">*</span>
                  </label>
                  <input
                    type="number"
                    required
                    ...
                    placeholder="20"
                    className="w-full bg-[#141215] text-white text-xs px-3.5 py-2.5 rounded-xl border border-[#29252A] focus:border-[#C9A34E] focus:outline-none font-bold text-[#C9A34E]"
                  />
                </div>

                <div>
                  <label className="block text-[11px] font-bold text-[#B0ACB0] uppercase mb-1">
                    Min. Lifetime Deposit (₹) <span className="text-[#C9A34E]">*</span>
                  </label>
                  <input
                    type="number"
                    required
                    ...
                    className="w-full bg-[#141215] text-white text-xs px-3.5 py-2.5 rounded-xl border border-[#29252A] focus:border-[#C9A34E] focus:outline-none font-bold text-[#C9A34E]"
                  />
                  <span className="text-[9px] text-[#777278] mt-0.5 block">Lifetime approved deposits requirement</span>
                </div>
              </div>
