...
export const CustomerSupportModal: React.FC<CustomerSupportModalProps> = ({
  ...
  const supportOptions = [
    ...
    },
    {
      ...
      badgeColor: 'bg-[#C9A34E]/20 text-[#C9A34E] border-emerald-500/40'
    },
    {
      id: 'instagram',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
      <div className="relative w-full max-w-lg bg-[#141215] border border-[#29252A] rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="p-5 border-b border-[#29252A] bg-gradient-to-r from-[#4A0D16] via-[#350A12] to-[#141215] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#C9A34E] to-[#8C6A24] flex items-center justify-center shadow-lg shadow-[#C9A34E]/20">
            ...
            <div>
              ...
              <p className="text-xs text-[#B0ACB0]/80 flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-[#C9A34E] inline" />
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl bg-[#1B181C] hover:bg-[#FF3048]/60 text-[#B0ACB0] hover:text-white transition active:scale-95"
        ...
        <div className="p-5 overflow-y-auto space-y-4 custom-scrollbar">
          {activeChannels.length === 0 ? (
            <div className="text-center py-12 px-4 rounded-2xl bg-[#141215] border border-[#29252A] space-y-3">
              <Headphones className="w-12 h-12 text-[#777278]/50 mx-auto animate-pulse" />
              <h3 className="text-sm font-extrabold text-[#C9A34E] uppercase">Support Desk Currently Offline</h3>
              <p className="text-xs text-[#B0ACB0]/80 max-w-xs mx-auto">
            ...
            activeChannels.map((item) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl bg-gradient-to-r ${item.bgGradient} border ${item.borderColor} bg-[#141215] hover:bg-[#1B181C] transition-all cursor-pointer group shadow-lg flex items-center justify-between gap-4 active:scale-[0.99]`}
                >
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm font-extrabold text-white group-hover:text-[#C9A34E] transition-colors">
                          {item.title}
                        </h4>
                      ...
                      <p className="text-xs text-[#B0ACB0]/80 mt-0.5 line-clamp-2">
                        {item.description}
                      </p>
                    </div>
                  </div>

                  <div className="p-2.5 rounded-xl bg-[#1B181C] group-hover:bg-amber-400 group-hover:text-black text-[#C9A34E] transition-colors flex-shrink-0 shadow-sm">
        ...
        {/* Footer */}
        <div className="p-4 border-t border-[#29252A] bg-[#100D21] flex items-center justify-between text-[11px] text-[#777278]">
          <span className="flex items-center gap-1">
            <ShieldCheck className="w-3.5 h-3.5 text-[#C9A34E]" /> Always verify official links before sharing credentials.
          </span>
          <button
            onClick={onClose}
            className="px-4 py-1.5 rounded-xl bg-[#141215]/50 hover:bg-[#FF3048]/60 text-[#B0ACB0] font-bold uppercase transition"
          >
            Close
          </button>
        </div>
